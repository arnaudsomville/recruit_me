...
def fill_gaps_in_template(
    template_path: Path,
    email_recipient_data: EmailRecipient,
    ...
    output_folder = Path.home().joinpath(
        f"{MainConfig().home_folder}/outputs/templates/{str(template_path.name.replace(f'.{extension}', ''))}/{str(uuid.uuid4())}"
    )
...
if __name__ == "__main__":
    ...
    fill_gaps_in_template(
        Path.home().joinpath(".recruit_me/template_examples/template_cover_letter.txt"),
        recipient,
    )
