...
class RecruitMe:
    ...
    def get_template_list(self) -> list:
        ...
        return sorted(
            [
                file.name
                for file in self.get_template_folder().iterdir()
                if file.is_file()
            ]
        )
    ...
    def get_cv_list(self) -> list:
        ...
        """
        return sorted(
            [file.name for file in self.get_cv_folder().iterdir() if file.is_file()]
        )
    ...
    def send_email(
        ...
        if cv_filename not in self.get_cv_list():
            raise ValueError(
                f"Provided cv '{cv_filename}' not in file list. Imported cvs are {' '.join(map(str, self.get_cv_list()))}"
            )

        if cover_letter_template_filename not in self.get_template_list():
            raise ValueError(
                f"Provided cover letter '{cover_letter_template_filename}' not in file list. Imported templates are {' '.join(map(str, self.get_template_list()))}"
            )

        if email_template_filename not in self.get_template_list():
            raise ValueError(
                f"Provided email '{email_template_filename}' not in file list. Imported templates are {' '.join(map(str, self.get_template_list()))}"
            )

        # Fill the templates
        filled_cover_letter_path = fill_gaps_in_template(
            self.get_template_folder().joinpath(cover_letter_template_filename),
            email_recipient,
            "Cover Letter",
        )
        filled_email_template_path = fill_gaps_in_template(
            self.get_template_folder().joinpath(cover_letter_template_filename),
            email_recipient,
