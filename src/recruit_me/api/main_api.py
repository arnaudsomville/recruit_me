...
from recruit_me.models.endpoint_models import (
    EmailContentEndpointModel,
    EmailSendingEndpointModel,
    UpdateAnswerEndpointModel,
)
...
def get_cv_list() -> dict:
    ...
    return {"status_code": STATUS_SUCCESS, "description": " ".join(map(str, cv_list))}
...
def update_answer(data: UpdateAnswerEndpointModel) -> dict:
    try:
        RecruitMe().update_response_status(data.email, data.new_answer)
    except Exception as e:
        return {
            "status_code": STATUS_INTERNAL_ERROR,
            "description": f"Internal error : {e}",
        }
    return {"status_code": STATUS_SUCCESS, "description": "Data updated"}
