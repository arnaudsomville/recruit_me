...
def test_txt_filling_nominal() -> None:
    ...
    with patch(
        "recruit_me.backend.template_filler.datetime"
    ) as mock_datetime, MainConfig().change_config_sources(DataSource(conf)):
        mock_datetime.now.return_value = datetime(2023, 11, 13)
        template_folder = Path(__file__).parent.joinpath("templates_for_tests")
        output_file: Path = fill_gaps_in_template(
            template_folder.joinpath("template_for_test.txt"), recipient
        )
        assert output_file.exists()
        with open(output_file, "r") as file_out, open(
            template_folder.joinpath("expected_filled_template_for_test.txt"), "r"
        ) as expected_file:
            output_content = file_out.read()
            expected_content = expected_file.read()
            assert (
                output_content == expected_content
            ), "The filled template does not match the expected output."
